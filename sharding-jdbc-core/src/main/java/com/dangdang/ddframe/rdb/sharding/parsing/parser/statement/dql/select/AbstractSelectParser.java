...
public abstract class AbstractSelectParser implements SQLStatementParser {
    ...
    public final SelectStatement parse() {
        ...
        sqlParser.getLexer().nextToken();     // get next token
        parseDistinct();                      // distinct
        parseBeforeSelectList();              // nada
        parseSelectList();                    // select item comma separated
        parseFrom();                          // !!! complex parse table
        parseWhere();                         // where
        customizedBetweenWhereAndGroupBy();   //
        parseGroupBy();                       // group by
    ...
    private void parseTable() {
        if (sqlParser.skipIfEqual(Symbol.LEFT_PAREN)) {
            ...
            sqlParser.skipUselessParentheses(); // skip (
            parse();                            // parse sub sql
            sqlParser.skipUselessParentheses(); // skip )
        ...
        customizedParseTableFactor();           // parse current table
        parseJoinTable();                       // parse next table
    }
    ...
    // parse join table or from next table
